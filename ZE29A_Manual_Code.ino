...
void setup() {
  ...
  Serial.println("\n=====================================");
  Serial.println(" 📖 공식 설명서 기반 음주측정기 시스템 시작");
      ...
      if (index == 9 && checkSum(packet) == packet[8]) {
        ...
        if (packet[1] == 0x85) {
          ...
          if (state != lastState) {
            lastState = state;
            switch(state) {
              case 0x31: Serial.println("[상태] 💤 대기 중... 자동으로 새 측정을 준비합니다."); break;
          ...
          // 상태에 따른 자동 액션
          if (state == 0x31) {
            // 센서가 아무것도 안 하고 놀고 있으면(0x31), 예열(0x32)을 시작하라고 명령!
