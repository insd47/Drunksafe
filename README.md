...
Drunksafe는 음주 후 사용자의 알코올 농도와 생체/상태 데이터를 측정하고, 개인별 알코올 분해 패턴을 바탕으로 위험도와 해독 예상 시간을 안내하는 모바일 연동 IoT 프로젝트입니다. 원본 한이음 드림업 문서의
Arduino/Flutter 기반 구상은 현재 구현 방향에 맞춰 **ESP32 DevKitC V4 + esp-rs 펌웨어**와 **React Native 모바일 앱**으로 갱신했습니다.

> 이 프로젝트는 자가 점검과 사고 예방을 돕는 보조 도구입니다. 의료 진단, 법적 음주 가능 여부 판단, 음주운전 가능 여부 판단을 대체하지 않습니다.
